...
segment DATA_SEGMENT
	...
	counter               DW   0      				   ; guardaremos cuantas iteraciones (divisiones) se hicieron
	counter_iter_fibos    DW   0      				   ; counter para controlar cuantas vueltas llevamos
	total_iters           DW   12      				   ; total de vueltas fibonaccis que queremos hacers
	counter_string_number DW   0      				   ; lo usaremos para desplazarnos byte a byte en  
...
fibonacci:								; Etiqueta de fibonacci, para cada iteración
	...
	XOR AX,AX          					; ponemos AX a 0
	MOV [counter],AX  					; volvemos a poner los counteres de string_number a 0 para resetearlos
	MOV [counter_string_number],AX 		; volvemos a poner los counteres de string_number a 0 para resetearlos
...
PRINT_NUMBER:
	...
	CONTINUE_DIV:							; ponemos AX a 0
		...
		MOV CX,[counter]   					; ponemos el valor de counter en CX
		INC CX              				; incrementamos CX para ir guardando cuantos numeros a mostrar van a la pila
		MOV [counter],CX     				; guardo en counter el valor de por donde vamos ( cuantos numeros llevamos )
	...
	PRINT_NUMBER_PROCESS:				; A partir de aqui vamos a ir sacando los digitos almacenados en la pila de forma inversa 
											; para ir almacenandolos en string_number, que es la variable que irá a la funcion 9H de INT21h
		XOR CX,CX				    		; Ponemos CX a 0
		MOV CX,[counter] 					; ponemos el counter de CX con el total de iteraciones de division
		...
		POP_MODULE: 						; Empezamos a sacar el resto
			...
			MOV AX,DATA_SEGMENT    		    ; guardamos los restos en la variable string_number para luego imprimirla
			MOV DS,AX       		    	; Nos situamos en el segmento de DATA_SEGMENT
			POP DX					    	; Sacamos 1 valor de la pila y lo guardamos DX , aquí estan los numeros
											; apilados de detras a delante 68322 (22386)
										
			ADD DX,'0'      		    	; DX contiene el numero guardado pero necesitamos pasarlos a CODE_SEGMENT ASCII
											; para ello sumamos el 0 (48 en decimal) y obtenemos el digito ASCII que equivale
											; a ese NUMERO
									
			MOV BX,[counter_string_number] 	; ponemos en BX el counter para string_number ( estará a 0 ). Es donde almacenaremos los 
											; numeros para luego pasar su segmento y desplazamiento a la funcion 9h int 21 
											; (imprimir string_number).

			MOV [string_number + BX],DX 	  ; Una vez situados en el segmento de DATA_SEGMENT accedemos a la variable string_number 
			...
			MOV [counter_string_number],BX ; guardamos en el counter su numero valor incrementado +1
			MOV BX,[counter_string_number]
			MOV AX,[counter]
			CMP AX,BX
		JNE POP_MODULE

		INC BX                      ; le sumo 1 más para añadir el caracter $ que indica fin de string_number 
		MOV AL,'$'                  ; añadimos el $ al final de la string_number ,uso AL para guardar el caracter $
		MOV [string_number + BX],AL 	    ; GUARDAMOS EL VALOR $ en el final de la string_number.
